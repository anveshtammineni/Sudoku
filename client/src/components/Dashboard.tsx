...
export function Dashboard({ data, loading }: { data: DashboardData | null; loading: boolean }) {
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-white/10 bg-white/5 p-5 shadow-glow backdrop-blur-xl dark:bg-slate-950/55">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-[0.25em] text-cyan-200/80">Player Dashboard</div>
          <h2 className="mt-1 text-xl font-semibold text-white">{data.user.name}</h2>
          <p className="text-sm text-slate-300">{data.user.email}</p>
        </div>
        <div className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-100">
          Live Profile
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {statCards.map((card) => {
          ...
          return (
            <div key={card.label} className="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <section>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-cyan-200/80">Recent Games</h3>
          <div className="space-y-3">
            {data.recentGames.length === 0 ? (
              <div className="rounded-2xl border border-white/10 bg-slate-950/25 p-4 text-sm text-slate-400">No saved games yet.</div>
            ) : (
        ...
        <section>
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-cyan-200/80">Leaderboard</h3>
          <div className="space-y-3">
            {data.leaderboard.map((entry, index) => (
              <div key={`${entry.userId}-${entry.difficulty}-${index}`} className="flex items-center justify-between rounded-2xl border border-white/10 bg-slate-950/25 p-4 text-sm text-slate-200">
                <div>
